...
XRayne - панель управления для работы с `xray-core`. Проект должен
предоставлять CLI, REST API и веб-интерфейс для локального управления Xray
на сервере по аналогии с 3x-ui.
...
- Отдельный Docker image `XRayne.Node` для удаленного инфраструктурного сервиса ноды
...
XRayne.Node/            REST API сервис удаленной инфраструктурной ноды
...
`XRayne.Node` - отдельный инфраструктурный REST API сервис для удаленного
сервера. Он собирается и публикуется отдельно от панели, не зависит от проектов
панели напрямую и на первом этапе предоставляет controller-based API каркас:

- API key авторизация через заголовок `X-Node-Api-Key`;
- файловое логирование;
- OpenAPI / Scalar документация;
- заглушки endpoints для управления `xray-core` и получения статуса системы.
...
- Подготовить Docker image `XRayne.Node` для публикации в GHCR.
