...
/// </summary>
public sealed class NodeProvisioner(
    ...
    IOptions<NodeConnectionOptions> connectionOptions) : INodeProvisioner
{
    /// <inheritdoc />
    public async Task<NodeProvisionResult> ProvisionAsync(
        NodeEntity node,
        string apiKey,
        string jobId,
        CancellationToken cancellationToken)
    {
        ...
        await UploadTextAsync(scp, installScriptPath, NodeInstallerScript.Content, cancellationToken);
        await UploadTextAsync(scp, configPath, NodeInstallerScript.RenderConfig(node, apiKey, image, connectionOptions.Value), cancellationToken);
        ...
        return new NodeProvisionResult(result.VerifiedAt);
    }
    ...
    private async Task InstallRemoteCoreAsync(
        ...
        CancellationToken cancellationToken)
    {
        var client = coreClientFactory.Create(new NodeEndpoint(node.Id, node.Address, node.ApiPort, apiKey));
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            ...
            if (state.Step is Node.Enums.InstallCoreStep.Installed)
            {
                return;
            }

            if (state.Step is Node.Enums.InstallCoreStep.Failure)
            {
...
internal static class NodeInstallerScript
{
