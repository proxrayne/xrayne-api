...
public sealed class NodeOutboundServiceTests
{
    ...
    [Fact]
    public async Task SyncReadonlyFromTemplateAsync_CreatesDisabledReadonly_WhenTagConflicts()
    {
        ...
        await outbounds.Received(1).AddAsync(
            AdminId,
            node.Id,
            Arg.Is<OutboundEntity>(outbound => outbound.ReadOnly && !outbound.Enabled && outbound.Tag == "direct"),
            Arg.Any<CancellationToken>());
    }
