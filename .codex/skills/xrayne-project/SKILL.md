...
- Prefer established folder boundaries: shared contracts and permissions in `XRayne.Contracts`, xray-core abstractions in `XRayne.Core`, implementations and utilities in `XRayne.Infrastructure`, EF persistence and external repository clients in `XRayne.Repositories`, HTTP in `XRayne.Api`, CLI commands in `XRayne.Cli`, frontend in `XRayne.UI`.
